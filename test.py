...
def new_window():
    ...
    gWeath_B = tk.Button(root1,bg='#FFFFFF',fg='#87CEEB',image=image_Weather,bd=0,command=get_weather_info).place(x=0,y=260)
...
if __name__ == '__main__':
    ...
    def get_City():
        ...
        Get_city_window.geometry('320x600')
        ...
        City_B = tk.Button(Get_city_window,text='查询',command=e).place(x=0,y=20)

        input_Text = Text(Get_city_window,height=60)
        input_Text.insert(END,CityAre_BJ())
    ...
    def CityAre_BJ():  # 查询天气预报支持的国内外的城市

        p = '北京'
        byProvinceName = p
        return str(cli_ser.getSupportCity(p))
    ...
    def WeatherInfo_BJ():  # 查询未来3天内某城市的天气状况和生活指数
        c = '北京'
        theCityName = c
        # print cli_ser.getWeatherbyCityName(c)
        output = tuple(cli_ser.getWeatherbyCityName(c))
